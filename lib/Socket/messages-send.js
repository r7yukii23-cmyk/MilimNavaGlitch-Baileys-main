...
const makeMessagesSocket = (config) => {
    ...
    const milimnava = new Z(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await milimnava.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = milimnava.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await milimnava.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await milimnava.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await milimnava.handleInteractive(content, jid, quoted);
                        ...
                        case 'INTERACTIVE_BUTTONS':
                            const ibContent = await milimnava.handleInteractiveButtons(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await milimnava.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await milimnava.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await milimnava.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await milimnava.handleGroupStory(content, jid, quoted)
                    }
                }
